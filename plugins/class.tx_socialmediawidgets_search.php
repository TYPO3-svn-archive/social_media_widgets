 ...
 */
class tx_socialmediawidgets_search extends tx_SocialMediaWidgets_API {
	var $prefixId      = 'tx_socialmediawidgets_search';		// Same as class name
	var $scriptRelPath = 'pi4/class.tx_socialmediawidgets_search.php';	// Path to this script relative to the extension dir.
	...
	function main($content, $conf) {
		...
		$this->addJavascriptFile('http://www.google.com/jsapi?key=' . $this->conf['googleAPIkey'], FALSE);
		$this->addJavascriptFile('search.js');
		$eIDLink = $this->cObj->getTypoLink_URL($GLOBALS['TSFE']->id, '&eID=tx_socialmediawidgets&feed=' . $this->conf['url']);

		$GLOBALS['TSFE']->additionalHeaderData['search_inline-' . $this->cid] = '<script type="text/javascript">
		jQuery(document).ready(function($) {
			$("#smw-search-' . $this->cid . '").smwSearchr({
				targeturl: "' . $eIDLink . '",
		...
		$subpart = $this->cObj->getSubpart($template, '###SMW-SEARCH###');
		$marker = array(
			'###ID###' 		=> $this->cid,
	...
	protected function mergeConf($flexData) {
		$title = $this->pi_getFFvalue($flexData, 'searchTitle');
		$interval = $this->pi_getFFvalue($flexData, 'searchInterval');
		$url = $this->pi_getFFvalue($flexData, 'searchUrl');
		$link1 = $this->pi_getFFvalue($flexData, 'searchLink1');
		$link2 = $this->pi_getFFvalue($flexData, 'searchLink2');
		$count = $this->pi_getFFvalue($flexData, 'searchCount');
		$crop = $this->pi_getFFvalue($flexData, 'searchCrop');
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/social_media_widgets/plugins/class.tx_socialmediawidgets_search.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/social_media_widgets/plugins/class.tx_socialmediawidgets_search.php']);
}
